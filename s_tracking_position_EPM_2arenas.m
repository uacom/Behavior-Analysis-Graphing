function [] = s_tracking_position_EPM_2arenas(filename,timeframe,interval_num,maxi,four) 
...
% e.g. s_tracking_position_2arenas_tf('filepath', [], 15, 0.15, 1);
% e.g. s_tracking_position_2arenas_tf('filepath', [2,5], 15, 0.15, 0);
% first argument filepath, second argument is timeframe, [] if whole trace.
...
% one meatball. 4th argument, maxi, is the max velocity displayed in the
% colorbar, and last argument, four, is 1 if 4 arenas, and 0 if two arenas.
% NOTE: for EPM, last argument should be 0 for two arenas.

%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%% THIS WORKS:

%%% s_tracking_position_EPM_2arenas('F:\dDrive_HP_Feb 2017on\DATA\By Person\S Qiu\Ethiovision\SQ behav trials\EPM 2 arena 02022019\Export Files\Raw data-QIULAB EPM 2 arena-Trial     1.xlsx',[], 30, 0.10,0);
...
scatter(p(1:num:dim,2),p(1:num:dim,3),15,p(1:num:dim,4),'filled'); colormap(flipud(jet));
...
%result=p;
%scatter(p(:,1),p(:,2),30,p(:,3),'square','filled');
scatter(p(1:num:dim,2),p(1:num:dim,3),15,p(1:num:dim,4),'filled');
% scatter(p(1:num:dim,2),p(1:num:dim,3),30,p(1:num:dim,4),'square','filled');
%
%
